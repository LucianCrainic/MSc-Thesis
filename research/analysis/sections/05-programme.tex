...
\figref{fig:programme} draws the ladder these stages climb.  Each rung
is unlocked by the evidence below it rather than promised in advance.
...
A fifth condition follows from the reframing in
\secref{sec:gap-software}.  Because the placement idea is already
formalised in simulation~\cite{fang2025kvplacement}, the stage must
state before implementation which of the differences in
\tabref{tab:rq4-delta} it is testing, and must report the measured
result against that prediction rather than against an unmanaged run
alone.  A stage that cannot name the prediction it tests has no
contribution left to make and should not be started.
...
The contribution is the combination of a justified abstraction, correct
placement, and a measured benefit or well-explained negative result
reported against the published prediction, rather than the proposal of
selective tiering, which is no longer available as a novelty claim.

\subsection{Deliverables and stopping rules}

\tabref{tab:deliverables} pairs each stage with what it must produce
and, more importantly, with the condition under which it should be
abandoned rather than pursued.
...
\addlinespace
4: implement
& Minimal selective-tier control, tests, ablation, and a measured
  comparison against the simulated prediction
...
Stage 3 strengthens the locality contribution.  Stage 4 adds a
hardware-verified test of a placement policy so far established only in
simulation, but it is not required for a coherent result.
