...
tensors should occupy HBM or how much improvement to expect.  That
second question has since been treated formally, though only in
simulation and only for KV state with weights pinned in the fast
tier~\cite{fang2025kvplacement}; \secref{sec:gap-software} sets out
what this leaves open.

\figref{fig:causal-chain} states the chain the experiment has to
observe rather than assume.  Every link in it is a place where a
plausible story could substitute for a measurement.
...
\tabref{tab:evidence-questions} separates the direct extension of
...
& Aurora HPL stages selected data between DDR and HBM; KV-only
  placement is formalised and bounded in
  simulation~\cite{fang2025kvplacement}.
& A CPU LLM runtime control and a hardware-measured ablation, including
  the weights-in-DDR case that the simulation excludes by
  assumption. \\
\addlinespace
Energy per token
