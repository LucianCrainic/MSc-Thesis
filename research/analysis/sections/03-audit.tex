...
\tabref{tab:runtime-audit} records what the inspected source does and
does not expose as a user-facing control.  The distinction the table
draws is deliberate: \emph{not exposed} is a statement about the
inspected path, not a claim that the behaviour is unreachable.
...
\(W\) and \(Q\) in the model of \secref{sec:synthesis}.  Build
